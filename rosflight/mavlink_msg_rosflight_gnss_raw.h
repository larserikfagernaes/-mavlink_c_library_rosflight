...
#define MAVLINK_MSG_ID_ROSFLIGHT_GNSS_RAW_MIN_LEN 84
#define MAVLINK_MSG_ID_198_LEN 84
#define MAVLINK_MSG_ID_198_MIN_LEN 84
                               ...
                               uint32_t time_of_week, uint16_t year, uint8_t month, uint8_t day, uint8_t hour, uint8_t min, uint8_t sec, uint8_t valid, uint32_t t_acc, int32_t nano, uint8_t fix_type, uint8_t num_sat, int32_t lon, int32_t lat, int32_t height, int32_t height_msl, uint32_t h_acc, uint32_t v_acc, int32_t vel_n, int32_t vel_e, int32_t vel_d, int32_t g_speed, int32_t head_mot, uint32_t s_acc, uint32_t head_acc, uint16_t p_dop, uint64_t rosflight_timestamp)
{
    ...
    return mavlink_finalize_message(msg, system_id, component_id, MAVLINK_MSG_ID_ROSFLIGHT_GNSS_RAW_MIN_LEN, MAVLINK_MSG_ID_ROSFLIGHT_GNSS_RAW_LEN, MAVLINK_MSG_ID_ROSFLIGHT_GNSS_RAW_CRC);
}
                                   ...
                                   uint32_t time_of_week,uint16_t year,uint8_t month,uint8_t day,uint8_t hour,uint8_t min,uint8_t sec,uint8_t valid,uint32_t t_acc,int32_t nano,uint8_t fix_type,uint8_t num_sat,int32_t lon,int32_t lat,int32_t height,int32_t height_msl,uint32_t h_acc,uint32_t v_acc,int32_t vel_n,int32_t vel_e,int32_t vel_d,int32_t g_speed,int32_t head_mot,uint32_t s_acc,uint32_t head_acc,uint16_t p_dop,uint64_t rosflight_timestamp)
{
    ...
    msg->msgid = MAVLINK_MSG_ID_ROSFLIGHT_GNSS_RAW;
    return mavlink_finalize_message_chan(msg, system_id, component_id, chan, MAVLINK_MSG_ID_ROSFLIGHT_GNSS_RAW_MIN_LEN, MAVLINK_MSG_ID_ROSFLIGHT_GNSS_RAW_LEN, MAVLINK_MSG_ID_ROSFLIGHT_GNSS_RAW_CRC);
}
...
static inline void mavlink_msg_rosflight_gnss_raw_send(mavlink_channel_t chan, uint32_t time_of_week, uint16_t year, uint8_t month, uint8_t day, uint8_t hour, uint8_t min, uint8_t sec, uint8_t valid, uint32_t t_acc, int32_t nano, uint8_t fix_type, uint8_t num_sat, int32_t lon, int32_t lat, int32_t height, int32_t height_msl, uint32_t h_acc, uint32_t v_acc, int32_t vel_n, int32_t vel_e, int32_t vel_d, int32_t g_speed, int32_t head_mot, uint32_t s_acc, uint32_t head_acc, uint16_t p_dop, uint64_t rosflight_timestamp)
{
    ...
    _mav_finalize_message_chan_send(chan, MAVLINK_MSG_ID_ROSFLIGHT_GNSS_RAW, buf, MAVLINK_MSG_ID_ROSFLIGHT_GNSS_RAW_MIN_LEN, MAVLINK_MSG_ID_ROSFLIGHT_GNSS_RAW_LEN, MAVLINK_MSG_ID_ROSFLIGHT_GNSS_RAW_CRC);
#else
    mavlink_rosflight_gnss_raw_t packet;
    packet.rosflight_timestamp = rosflight_timestamp;
    packet.time_of_week = time_of_week;
    ...
    _mav_finalize_message_chan_send(chan, MAVLINK_MSG_ID_ROSFLIGHT_GNSS_RAW, (const char *)&packet, MAVLINK_MSG_ID_ROSFLIGHT_GNSS_RAW_MIN_LEN, MAVLINK_MSG_ID_ROSFLIGHT_GNSS_RAW_LEN, MAVLINK_MSG_ID_ROSFLIGHT_GNSS_RAW_CRC);
#endif
}
 ...
 */
static inline void mavlink_msg_rosflight_gnss_raw_send_struct(mavlink_channel_t chan, const mavlink_rosflight_gnss_raw_t* rosflight_gnss_raw)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
    mavlink_msg_rosflight_gnss_raw_send(chan, rosflight_gnss_raw->time_of_week, rosflight_gnss_raw->year, rosflight_gnss_raw->month, rosflight_gnss_raw->day, rosflight_gnss_raw->hour, rosflight_gnss_raw->min, rosflight_gnss_raw->sec, rosflight_gnss_raw->valid, rosflight_gnss_raw->t_acc, rosflight_gnss_raw->nano, rosflight_gnss_raw->fix_type, rosflight_gnss_raw->num_sat, rosflight_gnss_raw->lon, rosflight_gnss_raw->lat, rosflight_gnss_raw->height, rosflight_gnss_raw->height_msl, rosflight_gnss_raw->h_acc, rosflight_gnss_raw->v_acc, rosflight_gnss_raw->vel_n, rosflight_gnss_raw->vel_e, rosflight_gnss_raw->vel_d, rosflight_gnss_raw->g_speed, rosflight_gnss_raw->head_mot, rosflight_gnss_raw->s_acc, rosflight_gnss_raw->head_acc, rosflight_gnss_raw->p_dop, rosflight_gnss_raw->rosflight_timestamp);
#else
    _mav_finalize_message_chan_send(chan, MAVLINK_MSG_ID_ROSFLIGHT_GNSS_RAW, (const char *)rosflight_gnss_raw, MAVLINK_MSG_ID_ROSFLIGHT_GNSS_RAW_MIN_LEN, MAVLINK_MSG_ID_ROSFLIGHT_GNSS_RAW_LEN, MAVLINK_MSG_ID_ROSFLIGHT_GNSS_RAW_CRC);
#endif
}
...
#else
        uint8_t len = msg->len < MAVLINK_MSG_ID_ROSFLIGHT_GNSS_RAW_LEN? msg->len : MAVLINK_MSG_ID_ROSFLIGHT_GNSS_RAW_LEN;
        memset(rosflight_gnss_raw, 0, MAVLINK_MSG_ID_ROSFLIGHT_GNSS_RAW_LEN);
    memcpy(rosflight_gnss_raw, _MAV_PAYLOAD(msg), len);
#endif
}
